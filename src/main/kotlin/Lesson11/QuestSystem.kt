...
class QuestSystem {
    ...
    fun register(){
        EventBus.subscribe { event ->
            ...
            // Проверка на завершения квеста после любого из полученых событий x
            // Проверяет что квест начался что начали разговор что убили цель что сообщили о завершении x
            // Выводите сообщение x
            // И отправлете в очередь событие Выполненного квеста x
            // Отмечаем флаг старта квеста как false x
        }
    }
}